...
streets <- c(str_sub(word(filename, 1), 6), word(filename, 2))
...
counts <- counts |> 
  mutate(vehicleType = if_else(vehicleType %in% c('heavyTruck','lightTruck'), 
                               'truck', vehicleType),
         dir = case_when(
           turning == 'left' ~ 'L',
           turning == 'right' ~ 'R',
         ...
         timestamp = as.POSIXct(timestamp, origin = "2001-01-01", 
                                tz = "America/New_York") + minutes(5) + seconds(30)
  ) |> 
  mutate(heading = factor(heading, c('SB','WB','NB','EB')),
         dir = factor(dir, c('Peds','R','T','L','U')),
         vehicleType = factor(vehicleType, c('car','truck','bicycle')))
...
all <- expand_grid(timestamp = timestamps,
                   vehicleType = factor(c('car','truck','bicycle')),
...
write_csv(counts_sum_peak, 
          paste0("out/", streets[1], '_', streets[2], '_', meridian, ".csv"))
